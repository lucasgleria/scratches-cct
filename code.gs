...
/**
 * Verifica se um HOUSE já existe para um determinado MAWB na planilha.
 * @param {string} spreadsheetId - O ID da planilha.
 * @param {string} mawb - O MAWB a ser verificado.
 * @param {string} house - O HOUSE a ser verificado.
 * @returns {object} - Retorna { ok: true, payload: { exists: boolean } } ou um erro.
 */
function checkHouseExists(spreadsheetId, mawb, house) {
  try {
    if (!spreadsheetId || !mawb || !house) {
      return _asError('Parâmetros inválidos para verificação de HOUSE.');
    }
    ...
    const lastRow = ws.getLastRow();
    if (lastRow === 0) {
      return _asOk({ exists: false });
    }

    const range = ws.getRange(1, 1, lastRow, 2).getValues(); // Colunas MAWB e HOUSE
    const normalizedMawb = _norm(mawb);
    const normalizedHouse = _normHouse(house);

    for (let i = 0; i < range.length; i++) {
      if (_norm(range[i][0]) === normalizedMawb && _normHouse(range[i][1]) === normalizedHouse) {
        return _asOk({ exists: true });
      }
    }

    return _asOk({ exists: false });
  } catch (err) {
    return _asError('Erro ao verificar a existência do HOUSE.', err.message);
  }
}